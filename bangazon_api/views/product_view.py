...
from bangazon_api.models import OrderProduct, Like
...
class ProductView(ViewSet):
    ...
    # on the Swagger site for the server (localhost:8000/swagger)
    @swagger_auto_schema(
    ...
    def like(self, request, pk):
        """Add a like for selected product and the current user"""
        try:
            product = Product.objects.get(pk=pk)
    ...
    def unlike(self, request, pk):
        ...
        try:
            product = Product.objects.get(pk=pk)
            like = Like.objects.get(customer=request.auth.user, product=product)
            like.delete()
            return Response(None, status=status.HTTP_204_NO_CONTENT)
